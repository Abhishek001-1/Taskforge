...
@router.put("/state")
def put_state(payload: StateIn, db: Session = Depends(get_session)):
    upsert_state(db, payload.model_dump())
    return {"ok": True}


@router.post("/state/import")
def import_state(payload: StateIn, db: Session = Depends(get_session)):
    upsert_state(db, payload.model_dump())
    return {"ok": True}
...
def reset_state(db: Session = Depends(get_session)):
    if not delete_state(db):
        raise HTTPException(404, "No state found")
    return {"ok": True}


@router.get("/days/{date}")
def get_day(date: str, db: Session = Depends(get_session)):
    day = read_state(db).get("days", {}).get(date)
    if not day:
...
def put_day(date: str, payload: DayIn, db: Session = Depends(get_session)):
    data = payload.model_dump(exclude_none=True)
    data.pop("date", None)
    return upsert_day(db, date, data)
