...
export default function Dashboard() {
  ...
  return <>
    <Header title="Today's Plan" kicker={format(new Date(), "EEEE, MMMM d")} />
    <Card className="mb-5 overflow-hidden bg-zinc-950 text-white dark:bg-white dark:text-zinc-950">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><p className="text-sm opacity-70">Today's focus</p><p className="mt-2 text-2xl font-bold">{quotes[new Date().getDate() % quotes.length]}</p></div><Link to="/daily" className="rounded-2xl bg-white px-4 py-2 text-sm font-bold text-zinc-950 dark:bg-zinc-950 dark:text-white">Open checklist</Link></div>
    </Card>
